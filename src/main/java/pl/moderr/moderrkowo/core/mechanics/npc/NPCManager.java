package pl.moderr.moderrkowo.core.mechanics.npc;
...
import pl.moderr.moderrkowo.core.mechanics.npc.data.npc.NPCDelayData;
import pl.moderr.moderrkowo.core.mechanics.npc.data.npc.NPCShopItem;
import pl.moderr.moderrkowo.core.mechanics.npc.data.npc.NPCShopItemSeasonOneCoins;
import pl.moderr.moderrkowo.core.mechanics.npc.data.quest.Quest;
import pl.moderr.moderrkowo.core.mechanics.npc.data.quest.QuestDifficulty;
import pl.moderr.moderrkowo.core.mechanics.npc.data.rewards.*;
...
//                                        add((IQuestRewardItemStack) () -> ItemStackUtils.addEnchantment(new ItemStack(Material.BOW, 1), Enchantment.ARROW_INFINITE, 1));
    ...
    public ItemStack getItemOfShop(User u, NPCData NPCData, NPCShopItem item) {
        ...
        if (unlocked) {
            ...
            if (item.getItem().getItemMeta().hasDisplayName()) {
                name = ColorUtils.color("&c&m" + PlainTextComponentSerializer.plainText().serialize(item.getItem().displayName()));
            } else {
                name = ColorUtils.color("&c&m" + ChatUtil.materialName(item.getItem().getType()));
            }
