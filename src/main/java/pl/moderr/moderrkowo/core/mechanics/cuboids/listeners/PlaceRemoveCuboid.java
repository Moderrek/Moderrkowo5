package pl.moderr.moderrkowo.core.mechanics.cuboids.listeners;
    ...
    public void cuboidPlace(BlockPlaceEvent e) {
        if (e.getItemInHand().isSimilar(CuboidsManager.getCuboidItem(1))) {
            ...
            if (regions.getRegion(CuboidsManager.getCuboidNamePrefix().toLowerCase() + e.getPlayer().getName().toLowerCase()) != null) {
                e.getPlayer().sendMessage(ModerrkowoPlugin.getServerName() + ColorUtils.color(" &cJuż masz jedną działkę w tym świecie!"));
            ...
            for (ProtectedRegion ignored : protectedRegionsCheck) {
                e.getPlayer().sendMessage(ModerrkowoPlugin.getServerName() + ColorUtils.color(" &cNie możesz postawić tutaj działki, znajduję się za blisko innej działki!"));
                e.getPlayer().playSound(e.getPlayer().getLocation(), Sound.ENTITY_VILLAGER_NO, 1, 1);
                e.setCancelled(true);
                return;
            }
            for (ProtectedRegion ignored : protectedRegions.getRegions()) {
                e.getPlayer().sendMessage(ModerrkowoPlugin.getServerName() + ColorUtils.color(" &cNie możesz postawić drugiej działki!"));
                e.getPlayer().playSound(e.getPlayer().getLocation(), Sound.ENTITY_VILLAGER_NO, 1, 1);
                e.setCancelled(true);
                return;
            }
            if (canBuild) {
                ...
                newCuboid.setFlag(Flags.DENY_MESSAGE,
                        ModerrkowoPlugin.getServerName() + ColorUtils.color(" &cNie masz uprawnień do interakcji na tej działce!"));
                ...
                e.getPlayer().sendMessage(ModerrkowoPlugin.getServerName() + ColorUtils.color(" &aPostawiłeś własną prywatną działkę! Aby zarządzać nią wpisz &2/dzialka"));
    ...
    public void cuboidDestroy(BlockBreakEvent e) {
        if (e.getBlock().hasMetadata("cuboid")) {
            if (e.getBlock().getMetadata("cuboid-owner").get(0).asString().equals(e.getPlayer().getName().toLowerCase())) {
                ...
                for (ProtectedRegion cuboid : set.getRegions()) {
                    if (cuboid.getId().startsWith(CuboidsManager.getCuboidNamePrefix())) {
                        ...
                        try {
                            ...
                            regions.save();
                            e.getPlayer().sendTitle(new Title(ModerrkowoPlugin.getServerName(), ColorUtils.color(" &aPomyślnie usunięto działkę!")));
                ...
                e.getPlayer().sendTitle(new Title(ModerrkowoPlugin.getServerName(), ColorUtils.color(" &cNie jesteś właścicielem tej działki! (" + e.getBlock().getMetadata("cuboid-owner").get(0).asString() + ")")));
                e.getPlayer().playSound(e.getPlayer().getLocation(), Sound.ENTITY_VILLAGER_NO, 1, 1);
