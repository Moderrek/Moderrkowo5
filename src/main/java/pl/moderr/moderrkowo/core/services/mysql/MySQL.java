...
public class MySQL {
    ...
    public void enable(String host, String port, String database, String username, String password) {
        ...
        try {
            synchronized (this) {
                ...
                Logger.logDatabaseMessage(ColorUtil.color("&aPołączono"));
                ...
                for (Player p : Bukkit.getOnlinePlayers()) {
                    if (!UserManager.isUserLoaded(p.getUniqueId())) {
                        ...
                        Logger.logDatabaseMessage(ColorUtil.color(MessageFormat.format("&a+ &2{0} &azostał wczytany", p.getName())));
                    }
                }
            ...
            Logger.logDatabaseMessage(ColorUtil.color("&cWystąpił problem SQL podczas łączenia"));
            e.printStackTrace();
        }
    }
    ...
    public void disable() {
        for (User u : UserManager.getUsers()) {
            ...
            Logger.logDatabaseMessage(ColorUtil.color(MessageFormat.format("&c- &4{0} &czostał odczytany", u.getName())));
        }
    }
